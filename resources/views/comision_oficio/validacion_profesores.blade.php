        ...
        <div class="row">
            <div class="col-md-9 col-md-offset-1">
                <ul class="nav nav-tabs">
                    <li>
                        <a href="{{url('/oficios/evaluacion/historialprofesores')}}">Historial de Oficios recibidos</a>
                    </li>
            ...
            <div class="col-md-10 col-md-offset-1">
                <table id="paginar_table" class="table table-bordered table-resposive">
                    ...
                    @foreach ($solicitudes as $oficio)
                        <tr>
                            <td rowspan="{{$oficio->comisionadosprofesores->count()+1}}">{{ $oficio->id_oficio }}</td>
                            <td rowspan="{{$oficio->comisionadosprofesores->count()+1}}">{{ $oficio->fecha_hora }}</td>
                            @foreach($oficio->usuario as $usuario)
                                ...
                                @foreach($usuario->abreviaciones_prof as $abreviacion)
                                    @foreach($abreviacion->abreviaciones as $titulo)
                                        <td rowspan="{{$oficio->comisionadosprofesores->count()+1}}"><b>{{ $titulo->titulo }}</b> {{ $usuario->nombre }}</td>
                                    @endforeach
                                @endforeach
                            @endforeach
                            {{--   <td rowspan="{{$oficio->comisionados->count()+1}}">{{ $oficio->desc_comision }}</td>--}}
                            @if($oficio->comisiones->count() ==0)
                            <td rowspan="{{$oficio->comisionadosprofesores->count()+1}}" class="text-center">
                        ...
                        @foreach($oficio->comisionadosprofesores as $ofic)
        ...
        $(document).ready(function() {
            ...
            $("#paginar_table").on('click','.modifica',function(){
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $("#form_modificar").attr("action","/oficios/comisionadosenviados/editarsubdirecion/"+idof)
