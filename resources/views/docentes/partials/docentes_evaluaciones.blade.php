                    ...
                    <div class="row col-md-12">
                        ...
                        <label for="" class="col-md-6 text-right">
                         ...
                         <label for="" class="col-md-12 text-right">
                             ...
                             <input type="{{ ($habilitaPDF==2 ? 'button' : 'hidden') }}" class="btn btn-primary tooltip-options link" id="link_genera_pdf" name="link_genera_pdf" data-token="{{ csrf_token() }}" data-toggle="tooltip" data-placement="top" title="Generar acta de calificaciones" target="_blank" value="Acta Ordinaria">
                        </label>
                    </div>
                </div>
            </div>
                                    ...
                                    @if(isset($uni_asignadas[$cont]))
                                        ...
                                        @if($anio<=$anio_v)
                                            @if($mes<=$mes_v)
                                                @if($dia<=$dia_v || $mes<$mes_v)
                                                    @if($id_unidad_res+$i==1)
                                                        <td class="text-center" ><input style="width: 7em" type="button" class="btn btn-primary tooltip-options link" id="guardar_calificacion" name="guardar_calificacion" data-token="{{ csrf_token() }}" data-id_unidad="{{$id_unidad_res+$i}}" data-toggle="tooltip" data-placement="top" title="La fecha limite para evaluar la unidad es {{ $dia_v }}-{{ $mes_v }}-{{ $anio_v }}" value="guardar" ></td>
                                                    @else
                                                        @if($evalPreviabtn)
                                                            <td class="text-center"><input style="width: 7em" type="button" class="btn btn-primary tooltip-options link" id="guardar_calificacion" name="guardar_calificacion" data-token="{{ csrf_token() }}" data-id_unidad="{{$id_unidad_res+$i}}" data-toggle="tooltip" data-placement="top" title="La fecha limite para evaluar la unidad es {{ $dia_v }}-{{ $mes_v }}-{{ $anio_v }}" value="guardar"></td>
