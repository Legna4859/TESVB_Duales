...
class In_pdf_grupoController extends Controller

{
    public function index($id_grupo,$id_nivel)
    {
        ...
        $nivel=DB::selectOne('SELECT * FROM `in_niveles_ingles` WHERE `id_niveles_ingles` = '.$id_nivel.'');
...
//dd($id_nivel);
        ...
        $pdf->Ln(3);
        $pdf->SetFont('Arial','','6.7');
        $pdf->Cell(170,3,utf8_decode('NIVEL: '.$nivel.' GRUPO:'.$grupo),0,0,'L');
        $pdf->Ln(3);
