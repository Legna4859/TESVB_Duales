...
class HrsGruposController extends Controller
{
    ...
    public function index()
    {
        ...
        $datos_carreras=array();    
            ...
            $grupos = DB::select('select DISTINCT m.id_semestre, hp.grupo ,c.id_carrera FROM 
                ...
                c.id_carrera ='.$carrera->id_carrera.'
            ...
            {
                $nombrem['grupo']= $grupo->grupo;
                $nombrem['id_semestre']=$grupo->id_semestre;
                array_push($nombre_grupos, $nombrem);
            }
            $nombre['grupos']=$nombre_grupos;
            array_push($datos_carreras,$nombre);
        }
        return view('horarios.horarios_grupos')->with('carreras',$datos_carreras);
    }
    public function horarios_grupos($id_semestre,$grupo,$id_carrera)
    {
        $id_periodo=Session::get('periodotrabaja');
...
$semanas=DB::select('select * FROM hrs_semanas ORDER by hora,id_semana');
        ...
        return view('horarios.horarios_grupos',compact('horario_grupo','carreras','id_carrera',
            'id_semestre2','grupo2','semanas','docentes','imprime'))->with(['ver' => true, 'carreras' => $datos_carreras]);
    }
