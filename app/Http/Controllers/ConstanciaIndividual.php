...
class ConstanciaIndividual extends Controller
{
   ...
   public function create()
    ...
    public function edit($id_registro_alumno)
    {
        ...
        $etiqueta=$etiqueta->descripcion;
            ...
            $personales=DB::selectOne('select DISTINCT actividades_complementarias.descripcion,actividades_complementarias.creditos,gnral_personales.nombre docente,abreviaciones.titulo,
                                        ...
                                        gnral_personales,gnral_alumnos,gnral_periodos,abreviaciones,abreviaciones_prof
                                        ...
                                        group by descripcion,periodo,nombre,apaterno,amaterno,cuenta,creditos,docente,titulo,promedio');
            $actividad=($personales->descripcion);
            $periodo=($personales->periodo);
            $nombre=($personales->nombre);
            $ap=($personales->apaterno);
            $am=($personales->amaterno);
            $cuenta=($personales->cuenta);
            $creditos=($personales->creditos);
            $docente=($personales->docente);
            $titulo=($personales->titulo);
            $promedio=($personales->promedio);
...
if($carrera==9)
        ...
        /// $texto = utf8_decode('El (la) que suscribe '.$abreviacion.' '.$nombre_jefe.', Jefe de División de la Carrera de '.$carrera_nombre.', por este medio me permito hacer de su conocimiento que el (la) estudiante  '.$nombre.' '.$ap.' '.$am.' con Número de Cuenta '.$cuenta.' ha cumplido su actividad complementaria '.$actividad.', con un nivel de desempeño '.$cuantitativa.'  y un valor numérico de '.$promedio.' durante el periodo escolar '.$periodo.' con un valor curricular de '.$creditos.' créditos.');
            ...
            $pdf->SetXY(10,-60); 
            $pdf->MultiCell(90,5,(''.utf8_decode($encarga).''.$jefe_carrera),0,'C',false);
            $pdf->SetXY(110,-70);           
