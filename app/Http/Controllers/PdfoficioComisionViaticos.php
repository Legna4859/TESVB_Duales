...
class PdfoficioComisionViaticos extends Controller
{
...
and gnral_municipios.id_estado=gnral_estados.id_estado and oc_depend_domicilio.id_oficio = '.$oficios->id_oficio.' ORDER BY id_depend_domicilio ASC');
        ...
        $pdf->SetFont('Arial','','8');
        $pdf->Cell(145);
        $pdf->Cell(30,4,utf8_decode('No. de Oficio '.$numero),0,0,'R');
        //fecha
        $fechas = date("Y-m-d");
        ...
        if($fechasalida == $fecharegreso) {
            ...
            $pdf->Ln();
            $pdf->SetFont('Arial', '', '8');
            $pdf->Cell(175, 5, utf8_decode('EL '.$fech1), 0);
        }
        else
        {
            $num1=date("j",strtotime($fechasalida));
