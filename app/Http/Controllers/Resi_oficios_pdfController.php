...
class PDF extends FPDF
{
    ...
    function Footer()
    {
        ...
        $this->Ln(4);
        $this->Cell(50);
        $this->Cell(145,-2,utf8_decode('FO-TESVB-93 V.0 23/03/2018'),0,0,'R');
        $this->Ln(3);
        $this->Cell(50);
...
class Resi_oficios_pdfController extends Controller
{
    public function index($id_anteproyecto){
        ...
        $pdf->SetX(80);
        $pdf->Cell(120,5,utf8_decode("OFICIO DE ACEPTACIÓN DE INFORME FINAL DE ASESOR INTERNO"),0,0,'C',true);
        $pdf->Ln(15);
        ...
        $pdf->MultiCell(190,5,utf8_decode("El suscrito, en mi carácter de asesor interno, designado por la Subdirección Académica, hago constar que después de haber revisado el Informe Final de Residencias Profesionales titulado:  "));
        $pdf->Ln(10);
        $pdf->MultiCell(190,5,utf8_decode($nombre_proyecto),0,'C');
        $pdf->Ln(10);
        if($promedio_general>=70)
        {
            $pdf->MultiCell(190,5,utf8_decode("       Presentado por el estudiante ".$dat_alumnos." cumple con los requisitos establecidos en el Reglamento para la acreditación de las Residencias Profesionales, con una calificación final de: ".$promedio_general));
        }
        else
        {
            $pdf->MultiCell(190,5,utf8_decode("       Presentado por el estudiante ".$dat_alumnos."  no cumple con los requisitos establecidos en el Reglamento para la acreditación de las Residencias Profesionales, con una calificación final de: ".$promedio_general));
        }
        $pdf->Ln(15);
        ...
        $pdf->Ln(15);
        $pdf->Cell(190,5,utf8_decode($asesor[0]->titulo.' '.$nombre_asesor),0,0,'C',FALSE);
        $pdf->Ln(5);
        $pdf->Cell(190,5,utf8_decode('Nombre y Firma del Asesor Interno'),0,0,'C',FALSE);
